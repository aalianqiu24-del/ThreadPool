...
private:
	...
	size_t m_waitTime = 10;        //等待时间 10ms
	...
	int Add(F&& f)//生产者
	{
		...
		while (!m_Stop && IsFull())
		{
			...
			if (std::cv_status::timeout == m_notFull.wait_for(locker, std::chrono::microseconds(m_waitTime)))
			{
				return -1;
			}
		}
...
public:
	...
	{
		return Add(std::forward<Task>(task));//1 2 3 4 
	}
